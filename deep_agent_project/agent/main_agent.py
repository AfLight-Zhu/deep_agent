...
from api.task_manager import is_cancelled, is_paused
import asyncio
import uuid
import shutil
...
safe_print(f"----------------project_root-----------------: {project_root}")
...
def _extract_message_content(content) -> str:
    """将 LangChain 消息 content（可能是 str 或 block 列表）规范化为纯文本。"""
    if content is None:
        return ""
...
def _resolve_task_result(last_result: str, thread_id: str) -> str:
    """优先使用当前任务生成的 Markdown，避免回退到会话目录中的历史报告。"""
    run_md = monitor.consume_latest_run_markdown(thread_id)
    trimmed = last_result.strip()

    if trimmed and run_md:
        # Agent 常只回复简短说明，完整报告在 Markdown 文件中
        if len(run_md) > len(trimmed) * 1.5:
            return run_md
        return trimmed
...
def _process_stream_chunk(chunk) -> str | None:
    """
    处理 LangGraph 流式输出的增量状态 (Stream Processing)。
    目标：
    1. 解析 Agent 的每一步思考和行动。
    2. 识别关键事件（工具调用、子 Agent 委派）。
    3. 收集最新的 AI 文本回复，流结束后统一上报。
    Args:
        chunk (dict): 增量状态字典，如 {"node_name": {"messages": [AIMessage(...)]}}
    Returns:
        str | None: 当前 chunk 中的 AI 文本回复（不含工具调用）
    """
    latest_text = None
    ...
    for node_name, state in chunk.items():
        ...
        if isinstance(messages, list) and messages:
            ...
            if isinstance(last_msg, AIMessage):
                ...
                # Case 2: Agent 生成文本回复（流结束后统一上报）
                elif last_msg.content:
                    text = _extract_message_content(last_msg.content)
                    if text:
                        latest_text = text
...
async def run_deep_agent(task_query: str, thread_id: str = None):
    ...
    safe_print(f"--- Start Task: {task_query} (Thread: {thread_id}) ---")
    ...
    try:
        ...
        async for chunk in main_agent.astream(
            ...
            if is_cancelled(thread_id):
                ...
                return "Cancelled"
            while is_paused(thread_id):
                if is_cancelled(thread_id):
                    monitor.report_task_cancelled()
                    return "Cancelled"
                await asyncio.sleep(0.3)
            # 实时处理每一个片段 (上报工具/子 Agent 进度，收集最终回复)
            chunk_text = _process_stream_chunk(chunk)
            if chunk_text:
                last_result = chunk_text

        # 流结束后统一上报最终结果
        final_result = _resolve_task_result(last_result, thread_id)
        if final_result:
            monitor.report_task_result(final_result)
        else:
            monitor.report_task_result("任务已完成，生成的文件可在右侧文件面板下载。")
        return "Done"
    except asyncio.CancelledError:
        monitor.report_task_cancelled()
        raise
    except Exception as e:
        # 7. [异常处理] 兜底捕获（含 recursion_limit 触顶）
        import traceback

        tb = traceback.format_exc()
        safe_print(f"Error: {e}\n{tb}")
        final_result = _resolve_task_result("", thread_id)
        if final_result:
            monitor.report_task_result(final_result)
        else:
            monitor._emit("error", f"Execution failed: {e}")
            monitor.report_task_result(f"任务执行异常: {e}")
        return f"Error: {e}"

    finally:
        monitor.clear_run_markdown_files(thread_id)
