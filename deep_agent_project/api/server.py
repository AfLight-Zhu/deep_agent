...
async def _ensure_utf8_encoding() -> None:
    """uvicorn 初始化 logging 后再次强制 UTF-8，避免 GBK handler 残留。"""
    bootstrap_encoding(force=True)
...
async def stop_task(request: StopTaskRequest):
    ...
    Args:
        request (StopTaskRequest): 包含 thread_id 的请求体。
    """
    stopped = request_cancel(request.thread_id)
    return {
...
@app.post("/api/task/stop-all")
async def stop_all_tasks():
    """停止所有正在执行的任务。"""
    stopped = request_cancel_all()
    return {
        "status": "stopped" if stopped else "not_found",
...
@app.get("/api/task/running")
async def get_running_tasks():
    """获取当前正在执行的任务列表。"""
    return {"running_tasks": list_running_tasks()}


@app.post("/api/task/pause")
async def pause_task(request: PauseTaskRequest):
    """暂停正在执行的任务。"""
    paused = request_pause(request.thread_id)
    if paused:
        token = set_thread_context(request.thread_id)
        try:
            monitor.report_task_paused()
        finally:
            _thread_id_ctx.reset(token)
    return {
        "status": "paused" if paused else "not_found",
        "thread_id": request.thread_id,
        "running_tasks": list_running_tasks(),
    }
...
async def resume_task(request: PauseTaskRequest):
    """恢复已暂停的任务。"""
    resumed = request_resume(request.thread_id)
    if resumed:
        token = set_thread_context(request.thread_id)
        try:
            monitor.report_task_resumed()
        finally:
            _thread_id_ctx.reset(token)
    return {
        "status": "resumed" if resumed else "not_found",
        "thread_id": request.thread_id,
        "running_tasks": list_running_tasks(),
    }
...
async def list_files(path: str):
    ...
    """
    # 1. [调试] 打印请求路径
    safe_print(f"[DEBUG] 请求文件列表: {path}")
    ...
    try:
        ...
        if not abs_path.is_relative_to(output_abs):
            safe_print(f"[ERROR] 拒绝访问: {abs_path} 不在 {output_abs} 目录下")
            return {"error": "拒绝访问: 只能访问输出目录下的文件"}

    except Exception as e:
        safe_print(f"[ERROR] 路径解析失败: {e}")
    ...
    try:
        ...
        safe_print(f"[ERROR] 遍历文件失败: {e}")
        return {"error": str(e)}

    # 6. [排序] 按修改时间倒序排列 (最新的在前)
    files.sort(key=lambda x: x.get("mtime", 0), reverse=True)
    safe_print(f"[DEBUG] 找到 {len(files)} 个文件")
    return {"files": files}
...
async def websocket_endpoint(websocket: WebSocket, thread_id: str):
    ...
    try:
        ...
        safe_print(f"[WebSocket] 客户端已断开: {thread_id}")

    except Exception as e:
        # 6. [异常] 发生错误时断开
        safe_print(f"[WebSocket] 连接异常: {e}")
        manager.disconnect(websocket, thread_id)
