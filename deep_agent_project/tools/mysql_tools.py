    ...
    try:
        ...
        with connect(**config) as conn:
            with conn.cursor() as cursor:
                ...
                monitor.report_tool_end("数据库获取表名工具", result)
                return result
    except Error as e:
        monitor.report_tool_end("数据库获取表名工具", str(e))
        return f"查询可用表名失败:{e}"
    ...
    try:
        with connect(**config) as conn:
            with conn.cursor() as cursor:
                ...
                header = ",".join(columns)
                output = f"{header}\n" + "\n".join(result)
                monitor.report_tool_end("数据库内容浏览工具", output)
                return output
    except Error as e:
        monitor.report_tool_end("数据库内容浏览工具", str(e))
    ...
    try:
        ...
        with connect(**config) as conn:
            with conn.cursor() as cursor:
                ...
                result = f"{header_str}\n" + "\n".join(rows_t)
                monitor.report_tool_end("数据库查询工具", result)
                return result
    except Error as e:
        monitor.report_tool_end("数据库查询工具", str(e))
